...
const EmailPage = () => {
  ...
  const { data: tabs, isLoading: tabsLoading, error: tabsError } = useQuery({
    queryKey: ['tabs'],
    queryFn: async () => {
      const response = await fetch('http://localhost:5001/api/tabs', {
        mode: 'cors',
      });
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      ...
      return data.map((tab) => ({
        ...tab,
        icon: iconMap[tab.icon], // Map icon string to component
      }));
    },
  });
  ...
  const { data: labels, isLoading: labelsLoading, error: labelsError } = useQuery({
    queryKey: ['labels'],
    queryFn: async () => {
      const response = await fetch('http://localhost:5001/api/labels', {
        mode: 'cors',
      });
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
  ...
  if (emailsLoading || tabsLoading || labelsLoading) {
    return (
  ...
  if (emailsError || tabsError || labelsError) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
        <Typography color="error">Error: {emailsError?.message || tabsError?.message || labelsError?.message}</Typography>
      </Box>
    );
  }
